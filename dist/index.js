            ...
            if (_this.keyDownTime === null) {
                ...
                if (newTime - _this.keyDownTime < _this.config.intervalBetweenKeyPress) { // если между нажатиями меньше 50 мс (у сканера примерно 25 мс)
                    ...
                    _this.isBusy = setTimeout(function () {
                        ...
                        _this.props.scanned({ data: _this.inputText, historyLength: _this.config.historyLength, historyDict: { username: 'John Doe' } });
                        _this.isBusy = null;
                        _this.inputText = '';
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (function (state, action) {
    var _a = action.payload, data = _a.data, historyLength = _a.historyLength, _b = _a.historyDict, historyDict = _b === void 0 ? {} : _b;
    state.isBusy = false;
    state.data = data;
    if (historyLength) {
        // @ts-ignore
        state.history.push(__assign(__assign({}, historyDict), { data: data, date: new Date().toISOString() }));
